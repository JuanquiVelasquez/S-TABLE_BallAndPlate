...
setpx=20
setpy=15
#Se establece el radio del circulo
R=20
...
while True:
    ...
    if len(cnts) > 0:
        ...
        if radius > 5:
            ...
            #estatico debe ser 0
            #Para generar el circulo se utilizan las funciones seno y coseno para obterner
            #la posicion deseada en x,y en el tiempo
            ux=-(Kx[0]*((re1/2)+(R*3/4*sin(math.radians(ta)))-x+8.5)+Kx[1]*(0-(x-xold)/(st-stold)))
            uy=-(Ky[0]*((re2/2)+(R*cos(math.radians(ta)))-y)+Ky[1]*(0-(y-yold)/(st-stold)))
            ...
            x1.append(t1)
            y1.append((re1/2)+(R*sin(math.radians(ta))))
            y2.append((re2/2)+(R*cos(math.radians(ta))))
            px.append(x)
            py.append(y)
            ta+=4
            t1 += 1
